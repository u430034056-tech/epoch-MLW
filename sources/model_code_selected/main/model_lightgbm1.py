...
def _build_lightgbm_feature_set(
    ...
    feature_set["numeric_features"] = feature_set["numeric_features"] + selected_extra_numeric_features
...
def train_model(
    ...
    """
    Train a leakage-aware LightGBM ensemble.

    Cross-validation uses fold-local preprocessing to avoid optimistic scores.
    Final training may still use the combined train/test GroupSize feature mode
    because the competition test PassengerId values are available at inference time.
    """
...
if __name__ == "__main__":
    ...
    model_path = save_model(result, "artifacts/lightgbm1_ensemble.joblib")
    submission_path = build_submission(result, output_path="artifacts/submission_lightgbm1.csv")
