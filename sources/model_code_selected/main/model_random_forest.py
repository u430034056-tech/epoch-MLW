"""Training and inference utilities for the Random Forest branch."""
...
def validate_rf_bundle(bundle: dict[str, Any]) -> dict[str, Any]:
    """Validate Random Forest bundle contents without changing preprocessing."""
...
def _build_model_config(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not isinstance(members, list) or not members:
        raise ValueError(f"[{MODEL_NAME}] Config must provide a non-empty 'members' list.")
    return model_config
...
def _build_model_payload(
    ...
    *,
    ...
    return {
        "model_type": "random_forest_probability_average_ensemble",
        "ensemble_policy": "mean_probability",
        "members": members,
...
def predict_proba(model: dict[str, Any], X: Any) -> np.ndarray:
    """Return averaged positive-class probabilities for the screened Random Forest ensemble."""
    members = model.get("members")
    if not isinstance(members, list) or not members:
        raise TypeError(f"[{MODEL_NAME}] Loaded ensemble does not contain any fitted member models.")

    probabilities: list[np.ndarray] = []
    for member in members:
        estimator = member.get("estimator")
        if not isinstance(estimator, RandomForestClassifier):
            raise TypeError(f"[{MODEL_NAME}] Ensemble member is not a RandomForestClassifier.")
        member_proba = np.asarray(estimator.predict_proba(X), dtype=float)
        if member_proba.ndim != 2 or member_proba.shape[1] < 2:
            raise ValueError(f"[{MODEL_NAME}] Random Forest member predict_proba must return two columns.")
        probabilities.append(member_proba[:, 1])
...
def train_model(
    ...
    """Train the screened Random Forest ensemble from the existing bundle."""
    ...
    X_subtrain, X_valid, y_subtrain, y_valid = train_test_split(
        ...
        stratify=validated["y_train"],
    )

    final_members: list[dict[str, Any]] = []
    validation_probabilities: list[np.ndarray] = []

    for member_config in model_config["members"]:
        member_name = str(member_config["name"])
        member_params = dict(member_config["params"])
        print(f"[{MODEL_NAME}] Training holdout member '{member_name}' with config: {member_params}")
        validation_estimator = RandomForestClassifier(**member_params)
        validation_estimator.fit(X_subtrain, y_subtrain)
        validation_probabilities.append(np.asarray(validation_estimator.predict_proba(X_valid), dtype=float)[:, 1])

        print(f"[{MODEL_NAME}] Re-fitting final member '{member_name}' on the full training bundle.")
        final_estimator = RandomForestClassifier(**member_params)
        final_estimator.fit(validated["X_train"], validated["y_train"])
        final_members.append({"name": member_name, "params": member_params, "estimator": final_estimator})

    validation_proba = np.mean(np.vstack(validation_probabilities), axis=0)
    validation_predictions = validation_proba >= float(threshold)
    validation_accuracy = float(accuracy_score(y_valid, validation_predictions.astype(int)))
    validation_log_loss = float(log_loss(y_valid, np.clip(validation_proba, 1e-6, 1 - 1e-6), labels=[0, 1]))

    model_payload = _build_model_payload(final_members, feature_names=validated["feature_names"])
    ...
    train_summary = {
        "summary_type": "holdout_validation_plus_full_refit",
        "mode": mode,
        "train_accuracy": float(np.mean(train_predictions == y_train_bool)),
        "validation_accuracy": validation_accuracy,
        "validation_log_loss": validation_log_loss,
        "validation_positive_rate_predicted": float(np.mean(validation_predictions)),
        "validation_positive_rate_observed": float(np.mean(y_valid)),
        "n_train_samples": int(validated["X_train"].shape[0]),
        "n_validation_samples": int(len(y_valid)),
        "n_test_samples": int(validated["X_test"].shape[0]),
        "ensemble_size": int(len(final_members)),
    }
    metadata = {
        "model_name": MODEL_NAME,
        ...
        "member_names": [member["name"] for member in final_members],
        "config": model_config,
    }
...
def save_model_artifacts(
    ...
    """Persist the screened Random Forest ensemble and metadata outside processed/."""
    ...
    feature_names = training_result["model"].get("feature_names")
    members = training_result["model"].get("members", [])
    if feature_names is not None and members:
        importance_arrays = []
        for member in members:
            estimator = member["estimator"]
            importance_arrays.append(np.asarray(estimator.feature_importances_, dtype=float))
        mean_importance = np.mean(np.vstack(importance_arrays), axis=0)
        if int(len(feature_names)) != int(len(mean_importance)):
...
def load_model_artifact_from_path(model_path: str | Path) -> tuple[dict[str, Any], Path]:
    """Load a trained Random Forest ensemble payload from an explicit path."""
    ...
    if model.get("model_type") != "random_forest_probability_average_ensemble":
        raise TypeError(f"[{MODEL_NAME}] Loaded artifact is not a screened Random Forest ensemble.")
    members = model.get("members")
    if not isinstance(members, list) or not members:
        raise TypeError(f"[{MODEL_NAME}] Loaded ensemble does not contain any member models.")
    for member in members:
        if not isinstance(member, dict) or not isinstance(member.get("estimator"), RandomForestClassifier):
            raise TypeError(f"[{MODEL_NAME}] Loaded ensemble contains an invalid member payload.")
...
def load_model_artifact(
    ...
    """Load a trained Random Forest artifact without triggering retraining."""
...
def run_inference(args: argparse.Namespace) -> dict[str, Any]:
    """Run strict bundle-based infer mode for the Random Forest branch."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the Random Forest model and generate a submission.")
