...
HANDOFF_DIR = config.PROJECT_ROOT / "next_kaggle_submission_queue"
...
def main() -> None:
    ...
    readme = [
        "# Next Kaggle Submission Candidates (Team Preprocessing Only)",
        "",
        "Hard constraint: every CSV here is generated from the team `processed/common` or `processed/xgboost` outputs.",
        "The Kaggle high-score template only contributes XGBoost parameters, SMOTE, seeds, thresholds, and positive-rate anchoring ideas.",
        "",
        "Priority is ranked by local OOF evidence and public-LB positive-rate experience:",
    ]
    ...
    readme.append("")
    readme.append("Note: this directory may retain candidates from an earlier team round; use the 1-8 order above for this round.")
    readme.append("")
    readme.append(f"Full manifest: `{manifest_path}`")
